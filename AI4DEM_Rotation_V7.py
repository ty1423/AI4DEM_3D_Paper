...
y_grid[0, 0, 1, 10, 10] = 10
z_grid[0, 0, 1, 10, 10] = 1
vx_grid[0, 0, 1, 10, 10] = 5
vy_grid[0, 0, 1, 10, 10] = 5
...
class AI4DEM(nn.Module):
    ...
    def forward(self, x_grid, y_grid, z_grid, vx_grid, vy_grid, vz_grid, angular_velocity_x, angular_velocity_y, angular_velocity_z, angular_x, angular_y, angular_z, fx_grid_collision, fy_grid_collision, fz_grid_collision, fx_grid_damping, fy_grid_damping, fz_grid_damping, mask, d, kn, damping_coefficient_Eta, friction_coefficient, diffx, diffy, diffz, diffvx, diffvy, diffvz, dt, input_shape, filter_size):
        cell_xold = x_grid / d
        cell_yold = y_grid / d 
        cell_zold = z_grid / d 
        ...
        for i in range(filter_size):
            for j in range(filter_size):
                for k in range(filter_size):
                    ...
                    vx_angular = diff_angular_velocity_y * moment_arm_z - diff_angular_velocity_z * moment_arm_y
                    vy_angular = diff_angular_velocity_z * moment_arm_x - diff_angular_velocity_x * moment_arm_z
                    vz_angular = diff_angular_velocity_x * moment_arm_y - diff_angular_velocity_y * moment_arm_x
                    ...
                    collision_torque_x = collision_torque_x + fz_grid_friction * moment_arm_y * mask - fy_grid_friction * moment_arm_z * mask
                    collision_torque_y = collision_torque_y + fx_grid_friction * moment_arm_z * mask - fz_grid_friction * moment_arm_x * mask
                    collision_torque_z = collision_torque_z + fy_grid_friction * moment_arm_x * mask - fx_grid_friction * moment_arm_y * mask
...
angular_velocity_x = torch.zeros(input_shape_global, device=device)
angular_velocity_y = torch.zeros(input_shape_global, device=device)
angular_velocity_z = torch.zeros(input_shape_global, device=device)
...
with torch.no_grad():
    for itime in range(1, ntime + 1):
        [x_grid, y_grid, z_grid, vx_grid, vy_grid, vz_grid, angular_velocity_x, angular_velocity_y, angular_velocity_z, angular_x, angular_y, angular_z, mask] = model(x_grid, y_grid,z_grid, vx_grid, vy_grid, vz_grid, angular_velocity_x, angular_velocity_y, angular_velocity_z, angular_x, angular_y, angular_z, fx_grid_collision, fy_grid_collision, fz_grid_collision, fx_grid_damping, fy_grid_damping, fz_grid_damping, mask, cell_size, kn, damping_coefficient_Eta, friction_coefficient, diffx, diffy, diffz, diffvx, diffvy, diffvz, dt, input_shape_global, filter_size)
