...
# Input Parameters
domain_size = 31  # Size of the square domain
half_domain_size = 15
...
class AI4DEM(nn.Module):
    ...
    def forward(self, x_grid, y_grid, z_grid, vx_grid, vy_grid, vz_grid, fx_grid_collision, fy_grid_collision, fz_grid_collision, fx_grid_damping, fy_grid_damping, fz_grid_damping, mask, d, kn, damping_coefficient_Eta, diffx, diffy, diffz, diffvx, diffvy, diffvz, dt, input_shape, filter_size):
        ...
        for i in range(filter_size):
            for j in range(filter_size):
                for k in range(filter_size):
                    ...
                    fn_grid = fn_grid_collision + fn_grid_damping
        ...
        fx_grid_left_friction_y      = - torch.where(is_left_overlap,    friction_coefficient * torch.abs (  fx_grid_boundary_left     - fx_grid_left_damping    )   * vy_grid / torch.maximum(eplis, torch.sqrt(vy_grid**2+vz_grid**2)), torch.tensor(0.0, device=device))
        fx_grid_left_friction_z      = - torch.where(is_left_overlap,    friction_coefficient * torch.abs (  fx_grid_boundary_left     - fx_grid_left_damping    )   * vz_grid / torch.maximum(eplis, torch.sqrt(vy_grid**2+vz_grid**2)), torch.tensor(0.0, device=device))

        fx_grid_right_friction_y     = - torch.where(is_right_overlap,    friction_coefficient * torch.abs (- fx_grid_boundary_right    - fx_grid_right_damping   )   * vy_grid / torch.maximum(eplis, torch.sqrt(vy_grid**2+vz_grid**2)), torch.tensor(0.0, device=device))
        fx_grid_right_friction_z     = - torch.where(is_right_overlap,    friction_coefficient * torch.abs (- fx_grid_boundary_right    - fx_grid_right_damping   )   * vz_grid / torch.maximum(eplis, torch.sqrt(vy_grid**2+vz_grid**2)), torch.tensor(0.0, device=device))
                
        fy_grid_bottom_friction_x    = - torch.where(is_bottom_overlap,   friction_coefficient * torch.abs (  fy_grid_boundary_bottom   - fy_grid_bottom_damping  )   * vx_grid / torch.maximum(eplis, torch.sqrt(vx_grid**2+vz_grid**2)), torch.tensor(0.0, device=device))
        fy_grid_bottom_friction_z    = - torch.where(is_bottom_overlap,   friction_coefficient * torch.abs (  fy_grid_boundary_bottom   - fy_grid_bottom_damping  )   * vz_grid / torch.maximum(eplis, torch.sqrt(vx_grid**2+vz_grid**2)), torch.tensor(0.0, device=device))
                
        fy_grid_top_friction_x       = - torch.where(is_top_overlap,      friction_coefficient * torch.abs (- fy_grid_boundary_top      - fy_grid_top_damping     )   * vx_grid / torch.maximum(eplis, torch.sqrt(vx_grid**2+vz_grid**2)), torch.tensor(0.0, device=device))
        fy_grid_top_friction_z       = - torch.where(is_top_overlap,      friction_coefficient * torch.abs (- fy_grid_boundary_top      - fy_grid_top_damping     )   * vz_grid / torch.maximum(eplis, torch.sqrt(vx_grid**2+vz_grid**2)), torch.tensor(0.0, device=device))
        
        fz_grid_forward_friction_x   = - torch.where(is_forward_overlap,  friction_coefficient * torch.abs (  fz_grid_boundary_forward  - fz_grid_forward_damping )   * vx_grid / torch.maximum(eplis, torch.sqrt(vx_grid**2+vy_grid**2)), torch.tensor(0.0, device=device))
        fz_grid_forward_friction_y   = - torch.where(is_forward_overlap,  friction_coefficient * torch.abs (  fz_grid_boundary_forward  - fz_grid_forward_damping )   * vy_grid / torch.maximum(eplis, torch.sqrt(vx_grid**2+vy_grid**2)), torch.tensor(0.0, device=device))
        
        fz_grid_backward_friction_x  = - torch.where(is_backward_overlap,  friction_coefficient * torch.abs (- fz_grid_boundary_backward - fz_grid_backward_damping)   * vx_grid / torch.maximum(eplis, torch.sqrt(vx_grid**2+vy_grid**2)), torch.tensor(0.0, device=device))
        fz_grid_backward_friction_y  = - torch.where(is_backward_overlap,  friction_coefficient * torch.abs (- fz_grid_boundary_backward - fz_grid_backward_damping)   * vy_grid / torch.maximum(eplis, torch.sqrt(vx_grid**2+vy_grid**2)), torch.tensor(0.0, device=device))
        ...
        vz_grid = vz_grid  + (dt / particle_mass) * ( - 0   * particle_mass) * mask + (dt / particle_mass) * ( - fz_grid_boundary_backward + fz_grid_boundary_forward - fz_grid_collision - fz_grid_damping - fz_grid_forward_damping - fz_grid_backward_damping + fz_grid_friction + fy_grid_bottom_friction_z  + fy_grid_top_friction_z       + fx_grid_left_friction_z  + fx_grid_right_friction_z ) * mask 
