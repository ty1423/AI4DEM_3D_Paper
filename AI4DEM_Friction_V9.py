...
class AI4DEM(nn.Module):
    ...
    def forward(self, x_grid, y_grid, z_grid, vx_grid, vy_grid, vz_grid, fx_grid_collision, fy_grid_collision, fz_grid_collision, fx_grid_damping, fy_grid_damping, fz_grid_damping, mask, d, kn, damping_coefficient_Eta, diffx, diffy, diffz, diffvx, diffvy, diffvz, dt, input_shape, filter_size):
        ...
        fn_grid = torch.zeros(input_shape, device=device) 
        ...
        for i in range(filter_size):
            for j in range(filter_size):
                for k in range(filter_size):
                    ...
                    fn_grid = fn_grid + torch.sqrt( (fx_grid_collision + fx_grid_damping) **2 + (fy_grid_collision + fy_grid_damping) **2 + (fz_grid_collision + fz_grid_damping) **2)
                    
                    # calculate the friction force between the two particles
                    diffv_Vt_x = diffv_Vt_x + diffvx * (diffy**2 + diffz**2) / torch.maximum(eplis, dist**2)
                    diffv_Vt_y = diffv_Vt_y + diffvy * (diffx**2 + diffz**2) / torch.maximum(eplis, dist**2)
                    diffv_Vt_z = diffv_Vt_z + diffvz * (diffx**2 + diffy**2) / torch.maximum(eplis, dist**2)
                    
                    diffv_Vt = torch.sqrt (diffv_Vt_x**2 + diffv_Vt_y**2 + diffv_Vt_z**2)
                    
                    fx_grid_friction = fx_grid_friction - torch.where(torch.lt(dist,2*d), friction_coefficient * fn_grid * diffv_Vt_x / torch.maximum(eplis,diffv_Vt), zeros)
