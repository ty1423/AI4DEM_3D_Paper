...
class AI4DEM(nn.Module):
    ...
    def forward(self, x_grid, y_grid, z_grid, vx_grid, vy_grid, vz_grid, fx_grid_collision, fy_grid_collision, fz_grid_collision, fx_grid_damping, fy_grid_damping, fz_grid_damping, mask, d, kn, damping_coefficient_Eta, diffx, diffy, diffz, diffvx, diffvy, diffvz, dt, input_shape, filter_size):
        ...
        for i in range(filter_size):
            for j in range(filter_size):
                for k in range(filter_size):
                    ...
                    fx_grid_friction = fx_grid_friction - torch.where(torch.lt(dist,2*d), torch.abs(torch.abs(friction_coefficient * fy_grid_collision) + torch.abs(friction_coefficient * fz_grid_collision) - friction_coefficient * fx_grid_damping) * diffvx / torch.maximum(eplis, torch.abs(diffvx)), zeros)
                    fy_grid_friction = fy_grid_friction - torch.where(torch.lt(dist,2*d), torch.abs(torch.abs(friction_coefficient * fx_grid_collision) + torch.abs(friction_coefficient * fz_grid_collision) - friction_coefficient * fy_grid_damping) * diffvy / torch.maximum(eplis, torch.abs(diffvy)), zeros)
                    fz_grid_friction = fz_grid_friction - torch.where(torch.lt(dist,2*d), torch.abs(torch.abs(friction_coefficient * fx_grid_collision) + torch.abs(friction_coefficient * fy_grid_collision) - friction_coefficient * fz_grid_damping) * diffvy / torch.maximum(eplis, torch.abs(diffvz)), zeros)
        ...
        vy_grid = vy_grid  + (dt / particle_mass) * ( - 0   * particle_mass) * mask + (dt / particle_mass) * ( - fy_grid_boundary_top      + fy_grid_boundary_bottom  - fy_grid_collision - fy_grid_damping - fy_grid_bottom_damping  - fy_grid_top_damping      + fy_grid_friction + fy_grid_bottom_friction  + fy_grid_top_friction     ) * mask 
        vz_grid = vz_grid  + (dt / particle_mass) * ( - 9.8 * particle_mass) * mask + (dt / particle_mass) * ( - fz_grid_boundary_backward + fz_grid_boundary_forward - fz_grid_collision - fz_grid_damping - fz_grid_forward_damping - fz_grid_backward_damping + fz_grid_friction + fz_grid_forward_friction + fz_grid_backward_friction) * mask 
