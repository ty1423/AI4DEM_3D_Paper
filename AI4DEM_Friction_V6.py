        ...
        for i in range(filter_size):
            for j in range(filter_size):
                for k in range(filter_size):
                    ...
                    diffv_Vt_x = diffvx * (diffy**2 + diffz**2) / dist**2
                    diffv_Vt_y = diffvy * (diffx**2 + diffz**2) / dist**2
                    diffv_Vt_z = diffvz * (diffx**2 + diffy**2) / dist**2
                    
                    diffv_Vt = torch.sqrt (diffv_Vt_x**2 + diffv_Vt_y**2 + diffv_Vt_z**2)
                    
                    fx_grid_friction = fx_grid_friction - torch.where(torch.lt(dist,2*d), friction_coefficient * torch.abs(- fx_grid_collision - fx_grid_damping) * diffv_Vt_x / torch.maximum(eplis,diffv_Vt), zeros)
                    fy_grid_friction = fy_grid_friction - torch.where(torch.lt(dist,2*d), friction_coefficient * torch.abs(- fy_grid_collision - fy_grid_damping) * diffv_Vt_y / torch.maximum(eplis,diffv_Vt), zeros)
                    fz_grid_friction = fz_grid_friction - torch.where(torch.lt(dist,2*d), friction_coefficient * torch.abs(- fz_grid_collision - fz_grid_damping) * diffv_Vt_z / torch.maximum(eplis,diffv_Vt), zeros)
