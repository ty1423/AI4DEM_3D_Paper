...
'''
x_grid[0, 0, 10, 12, 10] = 10
y_grid[0, 0, 10, 12, 10] = 12
z_grid[0, 0, 10, 12, 10] = 10
vx_grid[0, 0, 10, 12, 10] = 5
vy_grid[0, 0, 10, 12, 10] = -5
mask[0, 0, 10, 12, 10] = 1

x_grid[0, 0, 10, 10, 10] = 10
y_grid[0, 0, 10, 10, 10] = 10
z_grid[0, 0, 10, 10, 10] = 10
vx_grid[0, 0, 10, 10, 10] = 5
vy_grid[0, 0, 10, 10, 10] = 5
mask[0, 0, 10, 10, 10] = 1
'''
...
y_grid[0, 0, domain_size-3, j*2, i*2] = j * cell_size * 2
z_grid[0, 0, domain_size-3, j*2, i*2] = domain_size-3
vz_grid[0, 0, domain_size-3, j*2, i*2] = random.uniform(-1.0,1.0)
vx_grid[0, 0, domain_size-3, j*2, i*2] = random.uniform(-1.0,1.0)
...
class AI4DEM(nn.Module):
    ...
    def forward(self, x_grid, y_grid, z_grid, vx_grid, vy_grid, vz_grid, fx_grid_collision, fy_grid_collision, fz_grid_collision, fx_grid_damping, fy_grid_damping, fz_grid_damping, mask, d, kn, damping_coefficient_Eta, friction_coefficient, diffx, diffy, diffz, diffvx, diffvy, diffvz, dt, input_shape, filter_size):
        cell_xold = x_grid / d
        cell_yold = y_grid / d 
        cell_zold = z_grid / d 
...
with torch.no_grad():
    for itime in range(1, ntime + 1):
        [x_grid, y_grid, z_grid, vx_grid, vy_grid, vz_grid, mask] = model(x_grid, y_grid,z_grid, vx_grid, vy_grid, vz_grid, fx_grid_collision, fy_grid_collision, fz_grid_collision, fx_grid_damping, fy_grid_damping, fz_grid_damping, mask, cell_size, kn, damping_coefficient_Eta, friction_coefficient, diffx, diffy, diffz, diffvx, diffvy, diffvz, dt, input_shape_global, filter_size)
