...
class AI4DEM(nn.Module):
    ...
    def forward(self, x_grid, y_grid, z_grid, vx_grid, vy_grid, vz_grid, fx_grid_collision, fy_grid_collision, fz_grid_collision, fx_grid_damping, fy_grid_damping, fz_grid_damping, mask, d, kn, damping_coefficient_Eta, diffx, diffy, diffz, diffvx, diffvy, diffvz, dt, input_shape, filter_size):
        ...
        f_grid_collision = torch.zeros(input_shape, device=device) 
        ...
        for i in range(filter_size):
            for j in range(filter_size):
                for k in range(filter_size):
                    ...
                    f_grid_collision = f_grid_collision + kn * (dist - 2 * d )
                    ...
                    fx_grid_friction = - friction_coefficient * torch.abs(- fx_grid_collision - fx_grid_damping) * diffv_Vt_x / diffv_Vt
                    fy_grid_friction = - friction_coefficient * torch.abs(- fy_grid_collision - fy_grid_damping) * diffv_Vt_y / diffv_Vt
                    fz_grid_friction = - friction_coefficient * torch.abs(- fz_grid_collision - fz_grid_damping) * diffv_Vt_z / diffv_Vt
